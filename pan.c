...
/*** From source: modelo_verificacion_spin.pml ***/
...
short src_ln3 [] = {
	  0, 107, 108, 109, 110, 106, 112,   0, };
S_F_MAP src_file3 [] = {
	{ "-", 0, 0 },
	{ "modelo_verificacion_spin.pml", 1, 6 },
	{ "-", 7, 8 }
};
uchar reached3 [] = {
	  0,   1,   0,   0,   0,   0,   0,   0, };
...
short src_ln2 [] = {
	  0,  85,  87,  88,  91,  92,  93,  94, 
	 90,  97,  97,  98,  86, 100,  84, 102, 
	 84, 102, 103,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "modelo_verificacion_spin.pml", 1, 18 },
	{ "-", 19, 20 }
};
uchar reached2 [] = {
	  0,   1,   1,   0,   1,   1,   1,   1, 
...
short src_ln1 [] = {
	  0,  53,  55,  56,  57,  59,  20,  20, 
	 19,  18,  66,  67,  69,  71,  25,  24, 
	 75,  54,  77,  52,  78,  52,  78,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "modelo_verificacion_spin.pml", 1, 22 },
	{ "-", 23, 24 }
};
uchar reached1 [] = {
	  0,   1,   1,   0,   1,   1,   1,   0, 
...
short src_ln0 [] = {
	  0,  32,  33,  34,  35,  36,  31,  39, 
	 31,  39,  41,  42,  43,  44,  45,  40, 
	 47,  40,  47,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "modelo_verificacion_spin.pml", 1, 18 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 2:	/* Consumidor */
		...
		/* locals: */
		((P2 *)pptr(h))->r = 0;
		((P2 *)pptr(h))->workers_finalizados = 0;
#ifdef VAR_RANGES
		logval("Consumidor:r", ((P2 *)pptr(h))->r);
		logval("Consumidor:workers_finalizados", ((P2 *)pptr(h))->workers_finalizados);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 0:	/* Productor */
		...
		((P0 *)pptr(h))->i = 0;
#ifdef VAR_RANGES
		logval("Productor:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
		...
		now.en_critica = 0;
		total_unicos = 0;
		now.total_consumidos = 0;
		now.tareas = addqueue(calling_pid, 1, 0);
		now.procesado = addqueue(calling_pid, 2, 0);
#ifdef VAR_RANGES
		logval("mutex", now.mutex);
		logval("en_critica", now.en_critica);
		logval("total_consumidos", now.total_consumidos);
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.tareas) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.tareas) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.tareas) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.tareas) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.procesado) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.procesado) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.procesado) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.procesado) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan tareas (=%d):	len %d:\t", now.tareas, q_len(now.tareas));
	c_chandump(now.tareas);
	printf("	chan procesado (=%d):	len %d:\t", now.procesado, q_len(now.procesado));
	c_chandump(now.procesado);
	printf("	bit    mutex:	%d\n", now.mutex);
	printf("	byte   en_critica:	%d\n", now.en_critica);
	printf("	byte   total_consumidos:	%d\n", now.total_consumidos);
}
void
...
{	/* int i; */
	...
	case 3:
		...
		break;
	case 2:
		printf("local vars proc %d (Consumidor):\n", pid);
	printf("	byte   r:	%d\n", ((P2 *)pptr(pid))->r);
	printf("	byte   workers_finalizados:	%d\n", ((P2 *)pptr(pid))->workers_finalizados);
		break;
	case 1:
	...
	printf("	byte   t:	%d\n", ((P1 *)pptr(pid))->t);
		break;
	case 0:
		printf("local vars proc %d (Productor):\n", pid);
	printf("	byte   i:	%d\n", ((P0 *)pptr(pid))->i);
		break;
	}
}
